...
class MyPSACC:
    ...
    def __init__(self, refresh_token, client_id, client_secret, remote_refresh_token, customer_id, realm, country_code,
        ...
        self.last_stopped_state = datetime.now()
    ...
    def load_otp(self, force_new=False):
        ...
        if otp_session is None:
            ...
            if self.sms_code != "" and self.pin_code != "":
                otp_session = new_otp_session(smscode=self.sms_code, codepin=self.pin_code)
            else:
                logger.error("Code SMS / Pin code non saisi")
        elif force_new:
            logger.error("Renouvelez votre code OTP!!")
            #self.get_sms_otp_code()
            #otp_session = new_otp_session(self.sms_code, self.pin_code, otp_session)
    ...
    def load_config(name="config.json"):
        with open(name, "r") as f:
            ...
            if "country_code" not in config:
                config["country_code"] = "FR" #input("What is your country code ? (ex: FR, GB, DE, ES...)\n")
